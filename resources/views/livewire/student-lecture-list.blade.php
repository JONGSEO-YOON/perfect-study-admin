        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            @forelse ($lectures as $lecture)
                <a href="/lectures/{{ $lecture->id }}"
                    class="bg-white rounded-lg shadow-md p-5 hover:shadow-lg duration-300 cursor-pointer hover:brightness-90 transition-all">
                    <!-- 강사명 -->
                    <p class="text-gray-600 text-sm mb-2">{{ $lecture->user->name }} 강사</p>
                    <!-- 강의 제목 -->
                    <h3 class="text-lg font-semibold text-gray-800 mb-3 line-clamp-2">{{ $lecture->title }}</h3>
                    <!-- 대상 학년 & 강의 범위 -->
                    ...
                    <p class="text-sm text-gray-500 mb-4">
                        {{ $lecture->published_at?->format('Y.m.d') }} -
                        {{ $lecture->expired_at?->format('Y.m.d') }}
                    </p>
                    <!-- 진도율 -->
                    @php
                        function countVideosRecursively($items)
                        {
                            $count = 0;
                            foreach ($items as $item) {
                                ...
                                if (isset($item['children']) && is_array($item['children'])) {
                                    $count += countVideosRecursively($item['children']);
                                }
                            }
                            return $count;
                        }
                        $totalVideos = $lecture->lecture_info ? countVideosRecursively($lecture->lecture_info) : 0;

                        $watchedVideos = $lecture->videoViewHistories()->where('user_id', auth()->id())->count();
                        $progressPercent = $totalVideos > 0 ? ($watchedVideos / $totalVideos) * 100 : 0;
                    @endphp
                    <div class="flex items-center justify-between">
                        <div class="flex-1 mr-4">
                            <div class="w-full bg-gray-200 rounded-full h-1.5">
                                <div class="bg-[#7256C2] h-1.5 rounded-full" style="width: {{ $progressPercent }}%">
                                </div>
                            </div>
                        </div>
                        <span class="text-sm text-gray-600">{{ $watchedVideos }}/{{ $totalVideos }}</span>
                    </div>
                </a>
            @empty
